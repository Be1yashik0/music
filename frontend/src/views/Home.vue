  ...
  <v-container fluid>
    <v-app-bar app color="primary" dark class="rounded-app-bar">
      <v-btn icon @click="drawer = !drawer" class="menu-toggle rounded-btn" fixed top left>
        <v-icon v-if="!drawer"><img src="@/assets/icons/list.svg" alt="list" style="width: 24px; height: 24px;" /></v-icon>
        <v-icon v-else><img src="@/assets/icons/x.svg" alt="list" style="width: 24px; height: 24px;" /></v-icon>
      </v-btn>
    ...
    <v-navigation-drawer v-model="drawer" app temporary class="rounded-drawer">
      <v-list>
        ...
        <v-list-item to="/upload" v-if="authStore.user">
          ...
          <v-list-item-title>Загрузить музыку</v-list-item-title>
        </v-list-item>
      </v-list>
    </v-navigation-drawer>
      ...
      <v-card v-if="selectedTrack" class="rounded-card">
        ...
        <v-card-text>
          <div class="d-flex align-center">
            <v-btn icon @click="playTrack(selectedTrack)" class="rounded-btn ">
              <img src="@/assets/icons/play.svg" alt="play" style="width: 24px; height: 24px;" />
            </v-btn>
            <v-btn icon @click="playNext(selectedTrack)" class="rounded-btn ">
              <img src="@/assets/icons/next.svg" alt="list-music" style="width: 24px; height: 24px;" />
            </v-btn>
            <v-btn icon @click="addToQueue(selectedTrack)" class="rounded-btn ">
              <img src="@/assets/icons/queue.svg" alt="list-music" style="width: 24px; height: 24px;" />
            </v-btn>
            <v-btn icon @click="addToFavorites(selectedTrack)" class="rounded-btn ">
              <img src="@/assets/icons/add.svg" alt="favorite" style="width: 24px; height: 24px;" />
            </v-btn>
          </div>
          <div class="mt-4">
            ...
            <div class="d-flex align-center justify-space-between w-100">
              ...
              <div class="action-buttons">
                <v-btn icon @click="playAlbum(selectedAlbum, track)" class="rounded-btn buttn">
                  <img src="@/assets/icons/play.svg" alt="play" style="width: 18px; height: 18px;" />
                </v-btn>
                <v-btn icon @click="addToQueue(track)" class="rounded-btn buttn">
                  <img src="@/assets/icons/next.svg" alt="next" style="width: 18px; height: 18px;" />
                </v-btn>
                <v-btn icon @click="playNext(track)" class="rounded-btn buttn">
                  <img src="@/assets/icons/queue.svg" alt="queue" style="width: 18px; height: 18px;" />
                </v-btn>
                <v-btn icon @click="addToFavorites(track)" class="rounded-btn buttn">
                  <img src="@/assets/icons/add.svg" alt="favorite" style="width: 18px; height: 18px;" />
                </v-btn>
              </div>
            </div>
            </v-list-item>
    ...
    <Player v-if="isPlaying" :track="currentTrack" :isPlaying="isPlaying" @togglePlay="togglePlay" class="rounded-player" />    
  </v-container>
</template>
...
export default {
  ...
  data() {
    return {
      ...
      selectedTrack: null,
      selectedAlbum: null,
      currentTrack: { title: '', artist: '', audio_url: '' }, // Отдельная переменная для текущего воспроизводимого трека
      loading: false,
      isPlaying: false,
  ...
  methods: {
    async fetchTracks() {
      try {
        ...
        // Проверка доступности аудиофайлов
    ...
    async fetchAlbums() {
      try {
        ...
        albums.sort((a, b) => new Date(b.release_date) - new Date(a.release_date)) // Сортировка по убыванию даты
        this.albums = albums
      } catch (error) {
    ...
    showTrackDetails(track) {
      ...
      // Не трогаем isPlaying и currentTrack
    },
    ...
    playTrack(track) {
      ...
      this.isPlaying = false // Сбрасываем перед воспроизведением
      this.$nextTick(() => {
        this.isPlaying = true
      })
    },
